...
export const main = async (args: string[] = process.argv) => {
  ...
  program
    ...
    .option('-s, --scookie <scookie>', 'Set RPC secure cookie')
    .action((cmd) => {
      return cmdStart(cmd)
    })
  ...
  // COMMAND - balance
  program
    .command('newTx <from> <to> <amount> <txFee> <privateKey>')
    .description('Make a transaction')
    .usage('newTx <from> <to> <amount> <txFee> <privateKey>')
    .action((from, to, amount, txFee, privateKey, cmd) => {
