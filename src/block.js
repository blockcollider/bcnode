...
Block.prototype = {
  ...
  addEmblemDistance (emblems) {
    ...
    return big(Math.log(emblems))
      .times(Math.log(emblems))
      .toFixed(8)
  },
  ...
  addTransaction (tx) {
    ...
    const totalRange = self.txs.reduce((all, s) => {
      all = big(s.distance)
        .add(all)
        .toFixed(0)
    ...
    const rangeWithTx = big(tx.distance)
      .add(totalRange)
      .toFixed(0)

    const emblems = self.addEmblemDistance(self.emblems)

    const range = big(self.range)
      .add(emblems)
      .toFixed(0)
  ...
  addBlock (block_, forceAdd) {
    ...
    var block = _.cloneDeep(block_)

    const matches = self.blocks.filter(b => {
    ...
    if (matches.length === 0) {
      ...
      self.blocks = self.blocks.filter(b => {
        if (b.org !== block.org && b.tag !== block.tag) {
          return b
        }
      })
  ...
  addMerkleRoot () {
    ...
    const merkleRoot = self.txs.reduce((all, tx) => {
      if (all === '') {
        return tx.hash
      }
  ...
  addMutation (block) {
    ...
    const blocks = self.blocks.map(e => {
      if (e.org === block.org) {
        found = true
        ...
        if (e.input.indexOf('::') > -1) {
          ...
          self.distance = big(self.distance)
            .times(0.99)
            .toFixed(8)
        } else {
          e.input = `${e.input}::${block.input}`
    ...
    if (found === false) {
      ...
      const blocksFalse = blocks.filter(e => {
        if (e !== false) {
          return e
        }
      })
  ...
  addSignature (priv) {
    ...
    var _set = [self.miner, self.input, self.blockHash, self.merkleRoot]

    var set = string.blake2bl(_set.join(''))
  ...
  claimBlock (blockHash, minerPrivateKey) {
    ...
    const publicKey = crypt.createSecPublicKey(minerPrivateKey)

    const minerAddress = string.privateKeyToAddress(minerPrivateKey)

    const minerICAPAddress = string.addressToIcap(minerAddress)
