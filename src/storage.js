...
Storage.prototype = {
      ...
      if (err) {
        ...
        fs.ensureDir(self.bloomPath, function (err) {
          if (err) {
            ...
            if (
              fs.existsSync(self.genesisPath + '/' + self.genesisFile) == true
            ) {
            ...
            // self.genesis = require(self.genesisPath+"/"+self.genesisFile);
            self.writeMap = new Shared.Create(
              self.bloomPath + '/' + self.bloomFile,
              self.blockAlloc
            )
            self.readMap = new Shared.Open(
              self.bloomPath + '/' + self.bloomFile,
              self.blockAlloc
            )
  ...
  checkBloom: function (str) {
    ...
    while (++i < self.hashCycles) { if (self.writeMap[offsets[i]] != 1) return false }
    return true
  },
  ...
  getPrevBlock: function (block, cb) {
    ...
    self.db[block.id].findOne({ blockHash: block.data.prevHash }, function (
      err,
      res
    ) {
      if (err) {
  ...
  getBlockForkStatus: function (block, cb) {
    ...
    self.db[block.id].find({ prevHash: block.data.prevHash }, function (
      err,
      res
    ) {
      if (err) {
        ...
        if (res.length > 0) {
          ...
          self.getPrevBlock(block, function (err, prevBlock) {
            if (prevBlock._status == 'pending') {
              self.db[block.id].find({ prevHash: prevBlock.prevHash }, function (
                err,
                res
              ) {
                if (err) {
                  ...
                  self.db[block.id].remove(
                    {
                      prevHash: prevBlock.prevHash,
                      blockHash: { $ne: prevBlock.blockHash }
                    },
                    function (err) {
                      if (err) {
                        cb(err)
        ...
        if (err) {
          ...
          self.getBlockForkStatus(block, function (err, block) {
            if (err) {
              ...
              if (
                block.data.transactions != undefined &&
                global._RemoveBlockTransactions
              ) {
