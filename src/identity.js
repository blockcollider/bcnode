...
var time = require('./utils/time').default
...
function getDataDir () {
  switch (process.platform) {
    case 'win32':
      return process.env.LOCALAPPDATA + '/.blockcollider'
    case 'linux':
      return process.env.HOME + '/.blockcollider'
    case 'darwin':
      return process.env.HOME + '/Library/Application Support/.blockcollider'
    default:
...
identity.prototype = {
  saveToDisk: function (obj, cb) {
    fs.ensureDir(obj.identityPath, function (err) {
      if (err) {
        ...
        fs.writeFile(
          obj.identityPath + '/' + obj.identityFile,
          str,
          'utf8',
          function (err) {
            if (err) {
  ...
  getNetworkKey: function (currentIdentity, cb) {
    network.get_active_interface(function (err, active) {
      if (err) {
        ...
        if (
          currentIdentity.networkKey !== undefined &&
          currentIdentity.networkKey !== key
        ) {
          log.warn(
            'network key has changed ' +
  ...
  load: function (cb) {
    ...
    fs.pathExists(file, function (_, exists) {
      if (exists === true) {
