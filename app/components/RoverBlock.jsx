...
export class RoverBlock extends Component<*> {
  render () {
    const { network, block, blockchain, hash } = this.props

    return (
      <div className='card' style={{width: '8.4rem'}} onClick={() => inspectBlock(block)}>
        ...
        <div className='card-body'>
          ...
          <BlockLink network={network} blockchain={block.blockchain} hash={block.hash} />
        </div>
      </div>
    )
  }
...
const BlockLink = ({network, blockchain, hash}: {network: string, blockchain: string, hash: string}) => {
  let url = '#'
  switch (blockchain) {
    case 'btc':
  ...
  return <a href={url} target='_blank' className='btn btn-sm'>View block</a>
}
