...
Block.prototype = {
    ...
    addEdge: function(edge){
		...
		var edge = _.cloneDeep(edge);

        var matches = self.edges.filter(function(e){

            if(e.org == edge.org || e.tag == edge.tag){
                return e;
            }
	...
	addMutation: function(edge){
		...
		if(found == false){
			...
			if(self.edges.length != edgesFalse.length){

				self.edges = edgesFalse;
    ...
    claimBlock: function(proof, minerPrivateKey){
...
//tx.createHash();
//
//block.addEdge(ethEdge);
//block.addEdge(btcEdge);
