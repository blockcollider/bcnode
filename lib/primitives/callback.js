...
function Callback(options) {
  ...
  this.type = 3;  
  this.value = 0; 
  this.handler = null; // hash of the public key of the signature creating the transaction
...
Callback.prototype.fromOptions = function fromOptions(options){
    ...
    if (options.inputs) {
      ...
      for (const input of options.inputs)
        this.inputs.push(new CInput(inputs));
    }

    if (options.outputs) {
      assert(Array.isArray(options.outputs), 'Outputs must be an array.');
      for (const output of options.outputs)
        this.outputs.push(new COutput(output));
    }
