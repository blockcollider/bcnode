...
Object.setPrototypeOf(TX.prototype, TX.prototype);
...
TX.prototype.addInput = function addInput(options) {
...
TX.prototype.addOutpoint = function addOutpoint(outpoint) {
...
TX.prototype.addCoin = function addCoin(coin) {
...
TX.prototype.addTX = function addTX(tx, index, height) {
...
TX.prototype.addOutput = function addOutput(script, value) {
  let output;
...
TX.prototype.check = function check(flags) {
...
TX.prototype.checkAsync = function checkAsync(flags, pool) {
...
TX.prototype.verifyAsync = async function verifyAsync(flags, pool) {
  try {
...
TX.prototype.getSigopsSize = function getSigopsSize() {
...
TX.prototype.verifyInputs = function verifyInputs(height) {
...
TX.prototype.checkInputs = function checkInputs(height) {
...
TX.prototype.scriptInput = function scriptInput(index, coin, ring) {
...
TX.prototype.scriptVector = function scriptVector(prev, ring) {
  // P2PK
...
TX.prototype.signInputAsync = async function signInputAsync(index, coin, ring, type, pool) {
  if (!pool)
...
TX.prototype.signInput = function signInput(index, coin, ring, type) {
  const input = this.inputs[index];
...
TX.prototype.signVector = function signVector(prev, vector, sig, ring) {
  // P2PK
  const pk = prev.getPubkey();
  if (pk) {
...
TX.prototype.isSigned = function isSigned() {
...
TX.prototype.isInputSigned = function isInputSigned(index, coin) {
  const input = this.inputs[index];
...
TX.prototype.isVectorSigned = function isVectorSigned(prev, vector) {
  if (prev.isPubkey()) {
    if (vector.length !== 1)
      return false;
...
TX.prototype.template = function template(ring) {
...
TX.prototype.sign = function sign(ring, type) {
  if (Array.isArray(ring)) {
    let total = 0;
    for (const key of ring)
...
TX.prototype.signAsync = async function signAsync(ring, type, pool) {
  if (!pool)
...
TX.prototype.estimateSize = async function estimateSize(estimate) {
...
TX.prototype.selectCoins = function selectCoins(coins, options) {
...
TX.prototype.subtractFee = function subtractFee(fee, set) {
 ...
 * @param {Coin[]} coins
 * @param {Object} options - See {@link TX#selectCoins} options.
 * @returns {CoinSelector}
 */

TX.prototype.fund = async function fund(coins, options) {
...
TX.prototype.setSequence = function setSequence(index, locktime, seconds) {
  const input = this.inputs[index];
...
TX.prototype.format = function format() {
...
TX.prototype.getJSON = function getJSON(network) {
...
/**
 * Convert the TX to a TX.
 * @returns {TX}
 */

TX.prototype.toTX = function toTX() {
...
TX.prototype.commit = function commit() {
  return [this.toTX(), this.view];
};

/**
 * Instantiate TX from TX.
 * @param {TX} tx
 * @returns {TX}
 */

TX.fromTX = function fromTX(tx) {
  return new TX().inject(tx);
};

/**
 * Test whether an object is an TX.
 * @param {Object} obj
 * @returns {Boolean}
 */
